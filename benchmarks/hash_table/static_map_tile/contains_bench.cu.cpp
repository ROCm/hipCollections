 ...
 */
template <typename Key, typename Value, typename Dist, nvbench::int32_t TILESize>
std::enable_if_t<(sizeof(Key) == sizeof(Value)), void> static_map_contains(
  nvbench::state& state, nvbench::type_list<Key, Value, Dist, nvbench::enum_type<TILESize>>)
{
  ...
  cuco::static_map<Key, Value> map{
    size, cuco::empty_key<Key>{-1}, cuco::empty_value<Value>{-1}, TILESize};
...
NVBENCH_BENCH_TYPES(static_map_contains,
                    NVBENCH_TYPE_AXES(defaults::KEY_TYPE_RANGE,
                                      ...
                                      defaults::TILE_TYPE_RANGE))
  .set_name("static_map_contains_unique_occupancy")
  .set_type_axes_names({"Key", "Value", "Distribution", "TileSize"})
