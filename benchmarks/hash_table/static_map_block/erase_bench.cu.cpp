 ...
 */
template <typename Key, typename Value, typename Dist, nvbench::int32_t INPUTSize>
std::enable_if_t<(sizeof(Key) == sizeof(Value)), void> static_map_erase(
  nvbench::state& state, nvbench::type_list<Key, Value, Dist, nvbench::enum_type<INPUTSize>>)
{
  ...
  state.exec(
    nvbench::exec_tag::sync | nvbench::exec_tag::timer, [&](nvbench::launch& launch, auto& timer) {
      ...
      cuco::static_map<Key, Value, INPUTSize> map{
...
NVBENCH_BENCH_TYPES(static_map_erase,
                    NVBENCH_TYPE_AXES(defaults::KEY_TYPE_RANGE,
                                      ...
                                      defaults::INPUT_TYPE_RANGE))
  .set_name("static_map_erase_unique_occupancy")
  .set_type_axes_names({"Key", "Value", "Distribution", "INPUTSize"})
